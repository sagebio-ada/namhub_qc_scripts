...
| Category | Check | What it reports | Computed by | Verdict basis | Full file needed? |
|---|---|---|---|---|---|
| Metadata cross-check | `platform_vs_geo` | Synapse's `platform` annotation vs. GEO's own recorded sequencing instrument, after normalizing formatting differences between the two | GEO's record (fetched via the `geo-synapse` package) vs. Synapse's annotation | Objective correctness (string match) | No file at all |
| Metadata cross-check | `reference_vs_geo` | Synapse's `referenceSet` (reference genome) annotation vs. GEO's recorded genome assembly | GEO's record vs. Synapse's annotation | Objective correctness (substring match) | No file at all |
| Metadata cross-check | `library_version_vs_geo` | Synapse's `libraryVersion` (e.g. 10x Chromium chemistry version) annotation vs. GEO's own free-text description of the sequencing kit used | GEO's record vs. Synapse's annotation | Objective correctness (substring match) | No file at all |
| Metadata cross-check | `library_prep_method_vs_geo` | Synapse's `libraryPreparationMethod` annotation vs. GEO's own free-text kit description | GEO's record vs. Synapse's annotation | Objective correctness (substring match) | No file at all |
| Metadata cross-check | `geo_lookup` | Flags a raw run whose sample accession isn't found in the given GEO series at all | GEO's record (lookup only) | Objective correctness (lookup) | No file at all |
| Metadata cross-check | `fastq_size_vs_ena` | Downloaded file's byte size vs. the size ENA has registered for it | Downloaded file (custom code) vs. ENA's record | Objective correctness (exact match) | **Yes** |
| Metadata cross-check | `fastq_md5_vs_ena` | Downloaded file's checksum vs. the checksum ENA has registered for it | Downloaded file (custom code, MD5) vs. ENA's record | Objective correctness (exact match) | **Yes** |
| Metadata cross-check | `read_count_vs_ena` | FastQC's total-read count vs. the read count ENA has registered for this run | FastQC vs. ENA's record | Objective correctness (exact match) | **Yes** |
| Metadata cross-check | `mean_length_vs_registry` | The read length ENA's own read/base counts imply, alongside FastQC's directly observed read length(s) | FastQC and ENA's record, reported side by side | **Informational only** — no threshold, always `INFO` | **Yes** |
| Metadata cross-check | `paired_fastq_parity` | For paired-end runs stored as two separate mate files (not interleaved into one), whether both mates have the same number of reads | FastQC's per-file counts | Objective correctness (exact match) | **Yes** |
| Metadata cross-check | `matrix_dimensions` | Whether the matrix file's own header dimensions match the barcode/feature counts counted from the other two files | Custom code (via `scipy`/`numpy`) | Objective correctness (exact match) | **Yes** |
| Metadata cross-check | `kraken2_species_match` | % of reads Kraken2 actually classified as the organism GEO says this sample is | Kraken2 classification vs. GEO's record | **Informational only** — no threshold, always `INFO` | No |
...
| File-content check | `matrix_integrity` | Whether the count-matrix file is intact and parses as a valid matrix | Custom code (via `scipy`) | Objective correctness (parses or not) | **Yes** |
| File-content check | `matrix_metrics` | Median UMI (unique transcript molecule) count per cell barcode, median genes detected per cell barcode, how many barcodes have zero counts, total nonzero matrix entries | Custom code (via `numpy`) | Informational; only flagged as a real problem if the matrix is completely degenerate (zero non-empty barcodes at all) | **Yes** |
| File-content check | `empty_barcode_check` | What fraction of barcodes have zero counts, reported alongside whether Synapse declares this the raw (unfiltered) or filtered CellRanger output | Custom code (via `numpy`) | **Informational only** — no threshold, always `INFO` | **Yes** |
| File-content check | `mito_fraction` | Median fraction of each cell's counts coming from mitochondrial genes (a common per-cell quality signal — a very high fraction often indicates a dying/stressed cell), among barcodes with any counts at all | Custom code (via `numpy`) | **Informational only** — always `INFO` | **Yes** |
| Existence/resolution check | `fastq_resolution` | Whether the archived sequencing run's accession successfully resolved to a real, directly downloadable file, with the actual resolved URL(s) | ENA's public API | Objective correctness (did it resolve) | No file at all (API lookup only) |
...
| Existence/resolution check | `processed_completeness` | Whether all three required files (barcodes, features, matrix) are present for this sample, with the specific Synapse file ID behind each one | Synapse (lookup only) | Objective correctness (all 3 present) | No file at all |
| Existence/resolution check | `download:<role>` | Whether each of the three processed-data files downloaded successfully, with its Synapse file ID, filename, and size | Synapse | Objective correctness (did it succeed) | **Yes** — no capped-download option exists for these files; a full download always happens |
